...
def do_trajectory(goal):
	...
	# publish normally???
	left_arm_pub = rospy.Publisher('left_arm_controller/command', JointTrajectory, queue_size=10)
...
server = actionlib.SimpleActionServer('left_arm_controller/follow_joint_trajectory', FollowJointTrajectoryAction, do_trajectory, False)
server.start()
rospy.spin()
